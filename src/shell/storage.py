...
class BasicBlocks:
    ...
    def get_basic_blocks_set(self, address):
        basic_blocks = {}
        function = self.binary.get_function(address)
...
class FunctionMatchTool:
    ...
    def get_unidentified_blocks(self, function_match, source_function_address = 0, level = 0):
        ...
        src_binary = BasicBlocks(self.binaries[0])
        target_binary = BasicBlocks(self.binaries[1])
        src_basic_blocks = src_binary.get_basic_blocks_set(function_match.source)
        target_basic_blocks = target_binary.get_basic_blocks_set(function_match.target)
