...
#include "SQLiteDisasemblyReader.h"
#include "Log.h"

SQLiteDisasemblyReader::SQLiteDisasemblyReader(const char *DatabaseName)
{
    m_database = NULL;
    if (DatabaseName)
    {
...
SQLiteDisasemblyReader::~SQLiteDisasemblyReader()
{
    CloseDatabase();
}

bool SQLiteDisasemblyReader::Open(char *DatabaseName)
{
    m_databaseName = DatabaseName;
    return ConnectDatabase(DatabaseName);
}

bool SQLiteDisasemblyReader::ConnectDatabase(const char *DatabaseName)
{
    //Database Setup
    m_databaseName = DatabaseName;
...
const char *SQLiteDisasemblyReader::GetDatabaseName()
{
    return m_databaseName.c_str();
}

void SQLiteDisasemblyReader::CloseDatabase()
{
    //Close Database
    if (m_database)
    {
...
int SQLiteDisasemblyReader::ExecuteStatement(sqlite3_callback callback, void *context, const char *format, ...)
{
    int debug = 0;
...
int SQLiteDisasemblyReader::ReadBasicBlockDataCallback(void *arg, int argc, char **argv, char **names)
{
...
void SQLiteDisasemblyReader::ReadBasicBlockInfo(int fileID, char *conditionStr, AnalysisInfo *analysisInfo)
{
...
void SQLiteDisasemblyReader::Close()
{
    CloseDatabase();
}

int SQLiteDisasemblyReader::display_callback(void *NotUsed, int argc, char **argv, char **azColName)
{
    int i;
    for (i = 0; i < argc; i++) {
...
int SQLiteDisasemblyReader::ReadRecordIntegerCallback(void *arg, int argc, char **argv, char **names)
{
#if DEBUG_LEVEL > 2
...
int SQLiteDisasemblyReader::ReadRecordStringCallback(void *arg, int argc, char **argv, char **names)
{
#if DEBUG_LEVEL > 2
    printf("%s: arg=%x %d\n", __FUNCTION__, arg, argc);
    for (int i = 0; i < argc; i++)
...
int SQLiteDisasemblyReader::ReadFunctionAddressesCallback(void *arg, int argc, char **argv, char **names)
{
...
void SQLiteDisasemblyReader::ReadFunctionAddressMap(int fileID, unordered_set <va_t>& functionAddressMap)
{
...
char *SQLiteDisasemblyReader::ReadInstructionHash(int fileID, va_t address)
{
...
char *SQLiteDisasemblyReader::ReadName(int fileID, va_t address)
{
    char *name = NULL;
...
va_t SQLiteDisasemblyReader::ReadBlockStartAddress(int fileID, va_t address)
{
    va_t blockAddress;
...
int SQLiteDisasemblyReader::ReadMapInfoCallback(void *arg, int argc, char **argv, char **names)
{
...
multimap <va_t, PMapInfo> *SQLiteDisasemblyReader::ReadMapInfo(int fileID, va_t address, bool isFunction)
{
...
int SQLiteDisasemblyReader::ReadFunctionMemberAddressesCallback(void *arg, int argc, char **argv, char **names)
{
...
list<BLOCK> SQLiteDisasemblyReader::ReadFunctionMemberAddresses(int fileID, va_t function_address)
{
...
char *SQLiteDisasemblyReader::GetOriginalFilePath(int fileID)
{
...
char *SQLiteDisasemblyReader::ReadDisasmLine(int fileID, va_t startAddress)
{
...
int SQLiteDisasemblyReader::ReadBasicBlockCallback(void *arg, int argc, char **argv, char **names)
{
...
PBasicBlock SQLiteDisasemblyReader::ReadBasicBlock(int fileID, va_t address)
{
